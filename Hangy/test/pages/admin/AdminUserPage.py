...
class AdminUserPage(BasePage):
    ...
    DATA_USERNAME = By.ID, 'username'
    DATA_PASSWORD = By.ID, 'password'
    DATA_MAIL = By.ID, 'email'
    ...
    def create_new_user(self,username,password,email):
        self.typing(*self.DATA_MAIL, email + Keys.ENTER)
        self.typing(*self.DATA_USERNAME, username + Keys.ENTER)
        self.typing(*self.DATA_PASSWORD, password + Keys.ENTER)

    def alert(self):
        return self.find(*self.ALERT)

    def edit_user_username(self, kw):
        self.typing(*self.DATA_USERNAME, kw + Keys.ENTER)

    def edit_user_email(self, kw):
        self.typing(*self.DATA_MAIL, kw + Keys.ENTER)
